...
```c#
// XXXWrap.cs
L.RegFunction("Log", Log); // 向lua中添加Log方法。
```

**这里将要注册的方法转换成了供平台使用的指针，传递到C中生成lua函数。**
...
{
  	lua_pushstring(L, name); /* ->  压入name  top [name] bottom   */
...
{        
    lua_pushboolean(L, 0); /* - > 压入布尔值false top [0(false),name] bottom  */
    lua_pushcfunction(L, fn); /*  压入函数   */
...
```c
/* lua.h */
#define lua_pushcfunction(L,f)	lua_pushcclosure(L, (f), 0) /* 宏 压入一个闭包 */
...
[lua_pushcclosure ](https://pgl.yoyo.org/luai/i/lua_pushcclosure)
